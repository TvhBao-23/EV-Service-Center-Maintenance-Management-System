...
**Phạm vi:** Hồ sơ khách hàng, đăng ký xe, danh sách xe, đặt lịch hẹn  
**Kỹ thuật:** Phân vùng tương đương kết hợp phân tích giá trị biên  
**Công cụ thực thi:** Postman  
**Số test case sử dụng:** 13 test case  
...
2. Kiểm tra API đăng ký xe điện mới, đặc biệt là điều kiện bắt buộc và giá trị biên của VIN, năm sản xuất.
3. Kiểm tra API lấy danh sách xe của khách hàng.
4. Kiểm tra API đặt lịch hẹn sửa chữa/bảo dưỡng với xe, dịch vụ và trung tâm hợp lệ.
...
| Danh sách xe | Customer có xe hoặc không có xe | V_LIST | Lỗi server khi customer chưa có record | X_LIST_SERVER | Danh sách rỗng `[]` | B_LIST_EMPTY |
...
| Profile hợp lệ và cập nhật từng phần | TC01, TC02, TC03 |
