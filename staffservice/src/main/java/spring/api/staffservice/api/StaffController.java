...
public class StaffController {
    ...
    public ResponseEntity<?> updateChecklist(
        ...
        // Update fields
        if (updates.getBatteryHealth() != null) checklist.setBatteryHealth(updates.getBatteryHealth());
        if (updates.getBatteryVoltage() != null) checklist.setBatteryVoltage(updates.getBatteryVoltage());
        if (updates.getBatteryTemperature() != null) checklist.setBatteryTemperature(updates.getBatteryTemperature());
        if (updates.getBrakeSystem() != null) checklist.setBrakeSystem(updates.getBrakeSystem());
        if (updates.getTireCondition() != null) checklist.setTireCondition(updates.getTireCondition());
        if (updates.getTirePressure() != null) checklist.setTirePressure(updates.getTirePressure());
        if (updates.getLightsStatus() != null) checklist.setLightsStatus(updates.getLightsStatus());
        if (updates.getCoolingSystem() != null) checklist.setCoolingSystem(updates.getCoolingSystem());
        if (updates.getMotorCondition() != null) checklist.setMotorCondition(updates.getMotorCondition());
        if (updates.getChargingPort() != null) checklist.setChargingPort(updates.getChargingPort());
        if (updates.getSoftwareVersion() != null) checklist.setSoftwareVersion(updates.getSoftwareVersion());
        if (updates.getOverallStatus() != null) checklist.setOverallStatus(updates.getOverallStatus());
        if (updates.getNotes() != null) checklist.setNotes(updates.getNotes());
    ...
    public ResponseEntity<?> createMaintenanceReport(
            @RequestBody MaintenanceReport report,
            Authentication authentication) {
        
        User currentUser = (User) authentication.getPrincipal();
        report.setTechnicianId(currentUser.getUserId());
    ...
    public ResponseEntity<?> updateMaintenanceReport(
        ...
        if (updates.getWorkPerformed() != null) report.setWorkPerformed(updates.getWorkPerformed());
        if (updates.getPartsUsed() != null) report.setPartsUsed(updates.getPartsUsed());
        if (updates.getIssuesFound() != null) report.setIssuesFound(updates.getIssuesFound());
        if (updates.getRecommendations() != null) report.setRecommendations(updates.getRecommendations());
        if (updates.getLaborHours() != null) report.setLaborHours(updates.getLaborHours());
    ...
    public ResponseEntity<?> getConversation(
        ...
        User currentUser = (User) authentication.getPrincipal();
        List<spring.api.staffservice.dto.MessageDTO> messages = messageService.getConversation(currentUser.getUserId(), userId);
    ...
    public ResponseEntity<?> getUnreadMessages(Authentication authentication) {
        User currentUser = (User) authentication.getPrincipal();
        List<spring.api.staffservice.dto.MessageDTO> messages = messageService.getUnreadMessages(currentUser.getUserId());
