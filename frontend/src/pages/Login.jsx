...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_AUTH_API_URL || 'http://localhost:8081'}/api/auth/login`, {
        method: 'POST',
        headers: {
      ...
      // Decode JWT to get user info (simple decode, not verification for now)
      const tokenPayload = JSON.parse(atob(data.token.split('.')[1]))
      
      const userData = {
        id: tokenPayload.uid,
        fullName: tokenPayload.fullName || 'User',
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Chào mừng bạn quay trở lại EV Service Center
          </p>
        </div>
