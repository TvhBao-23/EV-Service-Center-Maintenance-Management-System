...
export const adminAPI = {
  ...
  getCustomers: () => {
    return apiCall(`${API_BASE_URLS.admin}/customers`, 'GET');
  },
  getCustomerDetails: (customerId) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/${customerId}`, 'GET');
  },
  createCustomer: (customerData) => {
    return apiCall(`${API_BASE_URLS.admin}/customers`, 'POST', customerData);
  },
  updateCustomer: (customerId, customerData) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/${customerId}`, 'PUT', customerData);
  },
  deleteCustomer: (customerId) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/${customerId}`, 'DELETE');
  },
  
  // === API XE (Vehicles) ===
  createVehicle: (customerId, vehicleData) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/${customerId}/vehicles`, 'POST', vehicleData);
  },
  updateVehicle: (vehicleId, vehicleData) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/vehicles/${vehicleId}`, 'PUT', vehicleData);
  },
  deleteVehicle: (vehicleId) => {
    return apiCall(`${API_BASE_URLS.admin}/customers/vehicles/${vehicleId}`, 'DELETE');
  },

  // === THÊM API NHÂN VIÊN (Staff) ===
  getAllStaff: () => {
    return apiCall(`${API_BASE_URLS.admin}/staff`, 'GET');
  },
  ...
  updateStaff: (staffId, staffData) => {
    // Make sure password is not sent if empty
    const payload = {...staffData};
    if (!payload.password) {
      delete payload.password;
    }
    return apiCall(`${API_BASE_URLS.admin}/staff/${staffId}`, 'PUT', payload);
  },
  deleteStaff: (staffId) => {
